...
import { motion } from 'framer-motion';
import { 
  ArrowRight, Phone, Mail, Server, ShieldCheck, Wrench, Truck, HardHat, Box, 
  RefreshCw, ServerOff, FileText, Users, Trash2, CheckCircle, Heart, Globe, 
  MapPin, Search, Edit, Upload
} from 'lucide-react';

// --- Admin Imports ---
import { useAuth } from '../context/AuthContext'; // UPDATE PATH AS NEEDED
import { dropOffApi } from '../api/dropOffApi'; // Use the new API
...
// --- Re-usable Sticky Sidebar (Refactored) ---
const ServicesSidebar = ({ donateData, isAdmin, onTextSave }) => {
  const otherServices = [
    { name: 'Electronics Recycling', path: '/services/recycling', icon: Globe },
    { name: 'IT Asset Disposition', path: '/services/itad', icon: Server },
    { name: 'Hard Drive Shredding', path: '/services/data-destruction', icon: ShieldCheck },
    // ... (rest of the static services)
    { name: 'Refurbishment', path: '/services/refurbishment', icon: Wrench },
    { name: 'Logistics & Pickup', path: '/services/logistics', icon: Truck },
    { name: 'On-site Services', path: '/services/on-site', icon: HardHat },
    { name: 'Equipment Buyback', path: '/services/equipment-buyback', icon: RefreshCw },
  ...
  return (
    <aside className="lg:sticky top-32 space-y-8">
      ...
      <FadeIn>
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 text-center shadow-sm">
          <DynamicIcon name={donateData.icon_name} className="w-10 h-10 text-blue-600 mb-4 mx-auto" />
          <EditableText
            as="h3"
            isAdmin={isAdmin}
...
// --- Main Content for this page (Refactored) ---
const DropOffContent = ({ overviewData, itemsData, isAdmin, onTextSave, onImageSave, onListEdit }) => {
  return (
    <div className="prose prose-lg max-w-none text-gray-700 space-y-16">
      {/* Section 1: Program Overview */}
      <FadeIn>
        <article>
          <EditableText
            ...
            onSave={onTextSave('program_overview', 'title')}
            className="text-3xl font-semibold text-gray-900 mb-6"
          />
          <EditableCenteredImage
            isAdmin={isAdmin}
            src={overviewData.image_url}
            alt={overviewData.image_alt}
            onSave={onImageSave('program_overview', 'image')}
          ...
          <EditableText
            as="p"
            isAdmin={isAdmin}
            value={overviewData.paragraph1}
            onSave={onTextSave('program_overview', 'paragraph1')}
            useTextarea
          />
          <EditableText
            as="p"
            isAdmin={isAdmin}
            value={overviewData.paragraph2}
            onSave={onTextSave('program_overview', 'paragraph2')}
            useTextarea
          />
        </article>
      </FadeIn>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
            <div>
              <EditableText
                as="p"
                isAdmin={isAdmin}
                value={itemsData.paragraph}
                onSave={onTextSave('accepted_items', 'paragraph')}
                className="mb-6"
                useTextarea
              />
              <div className="relative">
                {isAdmin && <AdminEditButton onClick={() => onListEdit('accepted_items_list')} text="Edit List" />}
                <ul className="space-y-3">
            ...
            <EditableCenteredImage
              isAdmin={isAdmin}
              src={itemsData.image_url}
              alt={itemsData.image_alt}
              onSave={onImageSave('accepted_items', 'image')}
              className="rounded-lg shadow-md w-full object-cover mt-6 md:mt-0"
              imgClassName="rounded-lg aspect-square"
            />
          </div>
          <EditableText
            as="p"
            isAdmin={isAdmin}
            value={itemsData.note}
            onSave={onTextSave('accepted_items', 'note')}
...
// --- How It Works Section (3-Step) (Refactored) ---
const HowItWorks = ({ data, isAdmin, onTextSave, onListEdit }) => {
  return (
    <section className="py-16 lg:py-24 bg-gray-50 relative">
      {isAdmin && <AdminEditButton onClick={() => onListEdit('how_it_works_steps')} text="Edit Steps" />}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <FadeIn>
          <EditableText
            as="h2"
...
// --- ADVANCED FEATURE: Interactive Map Section (Refactored) ---
const MapSection = ({ data, isAdmin, onTextSave, onImageSave, onListEdit }) => {
  return (
    <section className="py-16 lg:py-24 bg-white relative">
      {isAdmin && <AdminEditButton onClick={() => onListEdit('map_locations')} text="Edit Locations" />}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <FadeIn>
          <EditableText
            as="h2"
        ...
        <FadeIn>
          <div className="relative h-[500px] w-full rounded-lg bg-gray-200 shadow-inner overflow-hidden border border-gray-300">
            <EditableCenteredImage
              ...
              alt={data.bg_image_alt}
              onSave={onImageSave('map', 'bg_image')}
            ...
            {data.locations.map((loc, index) => (
              <motion.div
                key={loc.id}
                className="absolute"
                style={{ top: loc.top, left: loc.left }} // Assumes 'top' and 'left' are strings like '30%'
                initial={{ opacity: 0, y: -50 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5, delay: 0.5 + index * 0.2, type: 'spring', stiffness: 100 }}
              >
...
export const ElectronicsRecyclingDropOffBox = () => {
  ...
  const [modal, setModal] = useState(null); // 'hero_features', 'accepted_items_list', 'how_it_works_steps', 'map_locations'

  // --- Data Fetching ---
  useEffect(() => {
    const loadContent = async () => {
      try {
        const data = await dropOffApi.getContent();
        setContent(data);
      } catch (err) {
        console.error('Failed to load drop-off content', err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSave = async (sectionName, newContent) => {
    ...
    try {
      await dropOffApi.updateSectionContent(sectionName, newContent);
    } catch (err) {
  ...
  const handleMediaSave = (sectionName, field) => async (newFile) => {
    ...
    try {
      const { publicUrl, storagePath } = await dropOffApi.updateFile(newFile, oldStoragePath);
      const newContent = {
        ...section,
  ...
  const renderModalForm = (item, setItem) => {
    switch (modal) {
      ...
      case 'map_locations':
        return (
          <>
            <label>Location Name: <input type="text" className="w-full p-1 border rounded" value={item.name || ''} onChange={e => setItem({...item, name: e.target.value})} /></label>
            <label>Top Position (e.g., "30%"): <input type="text" className="w-full p-1 border rounded" value={item.top || ''} onChange={e => setItem({...item, top: e.target.value})} /></label>
            <label>Left Position (e.g., "25%"): <input type="text" className="w-full p-1 border rounded" value={item.left || ''} onChange={e => setItem({...item, left: e.target.value})} /></label>
          </>
        );
      default: return null;
    }
    ...
    <div className="bg-white text-gray-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-24">
        <div className="lg:grid lg:grid-cols-12 lg:gap-12">
          ...
          <div className="lg:col-span-8">
            <DropOffContent 
              overviewData={content.program_overview}
              itemsData={content.accepted_items}
              isAdmin={isAdmin}
          ...
          <div className="lg:col-span-4 mt-12 lg:mt-0">
            <ServicesSidebar 
              donateData={content.sidebar_donate}
              isAdmin={isAdmin}
              onTextSave={handleTextSave}
            />
          </div>
      ...
      {modal && (
        <ListManagementModal
          ...
          items={
            ...
            modal === 'accepted_items_list' ? content.accepted_items.items :
            modal === 'how_it_works_steps' ? content.how_it_works.steps :
            content.map.locations
          }
          itemTitleKey={
            modal === 'hero_features' ? 'title' :
            modal === 'accepted_items_list' ? 'text' :
            modal === 'how_it_works_steps' ? 'name' :
            'name'
          }
          onClose={() => setModal(null)}
          renderForm={renderModalForm}
          onSave={async (newItems) => {
            let sectionName = modal.split('_')[0]; // 'hero', 'accepted', 'how', 'map'
            let listKey = 'items'; // default
            
            if (modal === 'hero_features') {
              sectionName = 'hero';
              listKey = 'features';
            } else if (modal === 'accepted_items_list') {
              sectionName = 'accepted_items';
              listKey = 'items';
            } else if (modal === 'how_it_works_steps') {
              sectionName = 'how_it_works';
              listKey = 'steps';
            } else if (modal === 'map_locations') {
              sectionName = 'map';
              listKey = 'locations';
            }
